package cn.iocoder.yudao.module.restaurant.controller.admin.revenue.vo;

import cn.iocoder.yudao.framework.common.validation.InEnum;
import cn.iocoder.yudao.module.restaurant.enums.RevenueStatusEnum;
import io.swagger.v3.oas.annotations.media.Schema;
import jakarta.validation.constraints.*;
import lombok.Data;

import java.math.BigDecimal;
import java.time.LocalDate;

@Schema(description = "管理后台 - 营业收入创建/修改 Request VO")
@Data
public class RevenueSaveReqVO {

    @Schema(description = "营业收入编号", example = "1024")
    private Long id;

    @Schema(description = "门店编号", requiredMode = Schema.RequiredMode.REQUIRED, example = "1")
    @NotNull(message = "门店编号不能为空")
    private Long storeId;

    @Schema(description = "营业日期", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotNull(message = "营业日期不能为空")
    private LocalDate bizDate;

    @Schema(description = "收入来源", requiredMode = Schema.RequiredMode.REQUIRED, example = "meituan")
    @NotBlank(message = "收入来源不能为空")
    @Size(max = 50, message = "收入来源不能超过 50 个字符")
    private String source;

    @Schema(description = "收入金额", requiredMode = Schema.RequiredMode.REQUIRED, example = "10000.00")
    @NotNull(message = "收入金额不能为空")
    @DecimalMin(value = "0.01", message = "收入金额必须大于 0")
    @Digits(integer = 16, fraction = 2, message = "收入金额最多 16 位整数、2 位小数")
    private BigDecimal amount;

    @Schema(description = "状态，参见 RevenueStatusEnum 枚举", requiredMode = Schema.RequiredMode.REQUIRED, example = "0")
    @NotNull(message = "收入状态不能为空")
    @InEnum(value = RevenueStatusEnum.class, message = "收入状态必须是 {value}")
    private Integer status;

    @Schema(description = "备注", example = "美团外卖收入")
    @Size(max = 500, message = "备注不能超过 500 个字符")
    private String remark;

}